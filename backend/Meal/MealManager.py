...
class Meal_Manager(ManagerBase):
    ...
    def make_meal(self, meal_nutrient, meal_option, min_range, max_range):
        ...
        while not food_manager.carbs_full:
            ...
            # 단 지 탄 순으로 확인하면 된다.
            if meal_data["meal_protein"] > meal_nutrient["protein"] * min_range:
                setattr(food_manager, "protein_full",True)
                nutrient_number = 1
                if meal_data["meal_fat"] > meal_nutrient["fat"] * min_range:
                    setattr(food_manager, "fat_full",True)
                    nutrient_number = 2
                    if meal_data["meal_carbs"] > meal_nutrient["carbs"] * min_range:
